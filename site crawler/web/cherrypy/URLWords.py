    ...
    def savefollow(self, connectdb, level):
        ...
        if self.results:
            for res in self.results:
                curs.execute("INSERT INTO follow(link, level, idurl, idword, occurrencesfollow, percentagefollow) SELECT '"+self.address+"', '"+level+"', MAX(idurl) FROM url, MAX(idword) FROM words WHERE words.item = '"+res[2]+"''"+str(res[0])+"', '"+str(res[1])+"'")
        else:
